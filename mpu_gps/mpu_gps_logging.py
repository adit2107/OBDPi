...
file.write("Time,Ax,Ay,Az,Total acceleration,Latitude,Longitude\n")
...
while True:
	...
	print ("\tAx=%.2f g" %Ax, "\tAy=%.2f g" %Ay, "\tAz=%.2f g" %Az, "\tAt=%.2f g" %At,"\tLa=" ,La,"\tLo=" ,Lo)
	i=i+1
	now = datetime.now()
	file.write(str(now)+","+str(Ax)+","+str(Ay)+","+str(Az)+","+str(At)+","+str(La)+","+str(Lo)+"\n")
	file.flush()
	time.sleep(1)
file.close()
